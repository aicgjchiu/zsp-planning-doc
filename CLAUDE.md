...
- **Tabs:**
  ...
  - **`Enemies`** headers: `Id | Name | Map | Tier | HP | Damage | MoveSpeed | SpawnWeight | Behavior | Basis | Source | Hidden | SortOrder | CreatedAt | UpdatedAt | UpdatedBy`  — `Map` is a Maps-tab id; `Tier` is `trash / elite / boss`; `Basis` is `target` (designer-proposed) or `as-built` (verified in the UE 5.7 project). Themed display names carry as-built variant stats (e.g. Jiangshi 殭屍 = as-built `Zombie`). `Source` points at the file/asset in the game repo that owns the value (blank for design-only rows).
  - **`Gameplay`** headers: `Id | Section | Name | Value | Unit | Basis | Source | Notes | Hidden | SortOrder | CreatedAt | UpdatedAt | UpdatedBy`  — `Section` is `Pacing / Combat / Economy / Player / Abilities / Draft Cards / 質變 Cards / Vote Cards` (must match `GP_SECTIONS` in `app.js`; unknown sections render under an "Other" bucket). The Design Doc's run-segment banner computes from the well-known ids `gp-find-portal / gp-portal-cd / gp-activate / gp-farm-rate / gp-farm-len / gp-boss-rate / gp-boss-len` — renaming those ids breaks the model (it degrades to a "model needs numeric values" notice). `Source` points at the file/asset in the game repo that owns the value (blank for design-only rows).
...
- **Display-name standard (Spiria / Corrupia):** "soul energy" is the neutral spiritual resource; the **player** channels it as **Spiria** (靈能 — the "mana" stat; the Daoshi's Q is **Spiria Bullet · 靈能彈**), the **enemy** corrupts it into **Corrupia** (異能 — e.g. the Enemy Portal absorbs Corrupia to drive vote-upgrades / boss summons). Always use these display names in player-facing UI / design docs — never "Mana" / "Soul Power" for the player resource. Apply going forward; don't invent synonyms.
...
- **Leading `+` in cell values:** Sheets' `appendRow` formula-parses inputs — a Value like `+40~60` lands as `#ERROR!` even on text-formatted cells, and plus-prefixes on plain numerics get stripped. `handleUpsert`'s append path therefore writes rows via `setValues` (raw). If a batch of freshly appended rows ever shows `#ERROR!`, re-push the same values through the normal update path (`setValue` respects the text format) — that repairs them in place.
